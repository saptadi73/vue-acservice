...
import { useSalesOrderPdf } from '@/composables/useSalesOrderPdf'

const { generateSalesOrderPdfHtml, generatePdfFilename } = useSalesOrderPdf()
...
const showPrintButton = computed(() => ['confirmed', 'paid'].includes(form.value.status))
...
function generatePdfHtml() {
  return generateSalesOrderPdfHtml({
    orderNumber: form.value.order_number,
    orderDate: form.value.order_date,
    status: form.value.status,
    customer: form.value.customer,
    keterangan: form.value.keterangan,
    productLines: form.value.productLines,
    serviceLines: form.value.serviceLines,
    totals: totals.value,
    companyLogo,
    formatCurrency,
  })
...
function downloadPdf() {
  ...
  link.href = url
  link.download = generatePdfFilename(form.value.order_number, form.value.customer.nama)
  link.click()
