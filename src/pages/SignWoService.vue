          ...
          <table>
            <tbody>
              ...
              <tr>
                ...
                <td class="font-semibold text-gray-900">
                  <input id="checkSwing" type="checkbox" v-model="formData.check_swing" disabled />
                </td>
                <td class="pr-4 font-medium text-gray-700">
                  <input
...
async function signatureSend() {
  ...
  loadingStore.show()
  try {
    const response = await api.post(`wo/service/close/${customerAssetId}`, payload, {
...
async function getForNewWorkOrder(id) {
  loadingStore.show()
  try {
    const response = await axios.get(`${BASE_URL}wo/service/customercode/${id}`)
