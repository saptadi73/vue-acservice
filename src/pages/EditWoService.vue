            ...
            <tbody>
              <tr>
                ...
                <td class="font-semibold text-gray-900">
                  <!-- Debug: {{ daftarKeluhan.length }} keluhan available -->
    ...
    >
      <div class="w-[92vw] max-w-5xl h-[86vh] rounded-2xl bg-white p-4 shadow-xl flex flex-col">
        <div class="flex items-center justify-between mb-3">
          <h3 class="text-lg font-semibold text-gray-800">Preview WO Pemeliharaan</h3>
          <div class="flex items-center gap-2">
            <button
...
import { useWorkOrderPdfService } from '@/composables/useWorkOrderPdfService'
...
function closePdfPreview() {
  ...
  // Do not revoke object URL here; jsPDF returns bloburl, browser manages it
}
...
async function createWorkOrder() {
  loadingStore.show()
  try {
    console.log('📤 Submitting Work Order:', formData.value)
    ...
    show_toast.value = true
    message_toast.value =
      error.response?.data?.message || error.message || 'Gagal menyimpan work order'
  } finally {
    loadingStore.hide()
  }
}
...
// Helper: Convert absolute upload path to relative path for Vite proxy
...
// Upload handler for pelanggan signature
...
onMounted(async () => {
  ...
  await getForNewWorkOrder(customerAssetId)
  getPegawai()
})
...
async function previewPdfJsPdf() {
  const { generatePdf } = useWorkOrderPdfService()
  ...
  const blobUrl = await generatePdf(
    {
      ...
      keluhan: formData.value.keluhan || '-',
...
function downloadFromPreview() {
  ...
  a.download = `WO_Pemeliharaan_${no_wo.value || 'WO'}_${safeName}.pdf`
